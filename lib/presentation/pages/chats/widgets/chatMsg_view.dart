    ...
    useEffect(
      () {
        FirebaseMessaging.onMessage.listen((RemoteMessage message) {
          ...
          final isChat = jsonDecode(c['type']) as int == 0;
          if (isChat) {
            final d = c['conversation'];
            final decoded = jsonDecode(d) as Map<String, dynamic>;
            final chat = Chat.fromJson(decoded);

            Map<String, List<ChatMessage>> m = messages.value;
            final todayItems = m['Today'] ?? [];
            todayItems.add(chat.messages[0]);
            m['Today'] = todayItems;
            if (conversationId == chat.messages[0].conversation_id) {
              messages.value = m;
              messages.notifyListeners();
              if (chat.first_user_id != chat.messages[0].sender) {
                          ...
                          return Column(
                            children: [
                              ...
                              ...List.generate(group.length, (index) {
                               final d = getIt<IAuthLocalDatasource>();
                               final timezone = d.getTimezone();
                               final myLoc = tz.getLocation(timezone?? '');
                                return Column(
                                  children: [
                                    Padding(
                                              ...
                                              children: [
                                                ...
                                                Text(
                                                  DateFormat('h:mm a').format(
                                                      tz.TZDateTime.from(DateTime.parse(group[index].sent_on), myLoc)
                                                      // DateTime.parse(
                                                      //     group[index]
                                                      //         .sent_on)
                                                  ),
